...
export default function UserPanelCreate(props: UserPanelCreateProps) {
  ...
  const {
    ...
    formState: { errors: userDetailFormErrors, isValid: isUserDetailValid },
  } = useForm<UserModelInput>({
  ...
  const onSave: SubmitHandler<UserModelInput> = async (data) => {
    await saveUser(data);
    emitCloseDialog(true);
  };


  const saveUser = async (data: UserModelInput) => {
      ...
      <>
        ...
        <div className="mt-0 rounded-lg  px-4 py-0">
          ...
          <Button
            ...
            onClick={userDetailHandleSubmit(onSave)}
          >
