...
export function ViewUser(props: any) {
  ...
  const [getUserById, { data: adminUserData, refetch }] = useLazyQuery(GetUserById, {
    variables: {
      userId: '',
    },
    fetchPolicy: 'cache-and-network',
  ...
  useEffect(() => {
    ...
    setUserData(adminUserData ?? chwData);
    console.log(">>>", adminUserData ?? chwData)
  ...
  // SET EDIT FORMS
  useEffect(() => {
    if (userData && userDetailFormState) {
      userDetailSetValue('idNumber', userData?.idNumber, {
        shouldValidate: true,
      });

      userDetailSetValue('email', userData?.email, {
        shouldValidate: true,
      });
      userDetailSetValue('phoneNumber', userData?.phoneNumber, {
        shouldValidate: true,
      });
    }
    ...
    <div className="bg-red flex min-w-0 flex-col xl:flex">
      ...
      <div className="m-10 rounded-2xl lg:min-w-0 lg:flex-1">
            ...
            <div className="p-6 dark:bg-gray-900 dark:text-gray-100 sm:p-12">
              ...
              >
                ...
                <div className='sm: pt-12'>
                  <p className='text-3xl font-normal text-black '>{userData?.fullName}</p>
                  <div className="flex flex-row pt-4">
                    {userData && userData.roles.map((i: any, index: number) => {
                      return <div
                        key={i.id}
            ...
            <form key={"formKey"} className="space-y-8 divide-y divide-gray-200">
              {
                editActive ?
                  <>
                    <div className="space-y-0">
                      {props.location.state?.component === 'chw' && <>
                        <p className='text-md py-2 mt-4'>Which kind of identification do you have for {userData?.firstName}?</p>
                  ...
                  (props.location.state?.component === 'administrators' && props.location.state?.component !== 'team-leads') ? <div className='flex flex-row justify-start pt-4 text-current'>
                    <p className='text-xl px-4'>Email: {userData?.email}</p>
                  </div> : <div className='flex flex-row justify-start pt-4 text-current'>
                    <p className='text-xl px-4'>ID: {userData?.idNumber}</p>
                    <p className='text-xl px-4'> Cellphone: {userData?.phoneNumber}</p>
                    <p className='text-xl px-4'>WhatsApp: {userData?.phoneNumber}</p>
                  </div>
              }
            </form>
            {/* End main area */}
        ...
        <div className="pl-4 flex flex-row w-full justify-between">
                ...
                dialog({
                  ...
                  render: (onSubmit: any, onCancel: any) => (
                    <AlertModal
                      ...
                      message={`${userData?.firstName} will lose their access to AppName immediately. Make sure you have communicated with them before deactivating them.`}
                      onCancel={onCancel}
                      onSubmit={() => {
                        onSubmit();
                        deleteUser({
          ...
          <p className="text-gray-500 mt-3 text-sm">User added to {data?.tenantContext.applicationName}: {userData?.insertedDate}</p>
