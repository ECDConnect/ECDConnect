...
export function ViewUser(props: any) {
  ...
  useEffect(() => {
    if (userData?.userById && chwData?.GetHealthCareWorkerById.user && userDetailFormState) {
      userDetailSetValue('idNumber', userData?.userById?.idNumber ?? chwData?.GetHealthCareWorkerById?.user?.user.idNumber, {
        shouldValidate: true,
      });

      userDetailSetValue('email', userData?.userById?.email ?? chwData?.GetHealthCareWorkerById?.user?.user.email, {
        shouldValidate: true,
      });
      userDetailSetValue('phoneNumber', userData?.userById?.phoneNumber ?? chwData?.GetHealthCareWorkerById?.user?.user.phoneNumber, {
        shouldValidate: true,
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userData?.userById, chwData?.GetHealthCareWorkerById?.user?.user]);
    ...
    <div className="bg-red flex min-w-0 flex-col xl:flex">
      ...
      <div className="m-10 rounded-2xl lg:min-w-0 lg:flex-1">
        <div className="py-0 px-4 sm:px-6 lg:px-8">
            ...
            <div className="p-6 dark:bg-gray-900 dark:text-gray-100 sm:p-12">
              ...
              >
                ...
                <div className='sm: pt-12'>
                  <p className='text-3xl font-normal text-black '>{userData?.userById?.fullName ?? chwData?.GetHealthCareWorkerById?.user?.fullName}</p>
                  <div className="flex flex-row pt-4">
                    {userData?.userById && userData?.userById.roles.map((i: any, index: number) => {
                      return <div
                        key={i.id}
                    ...
                    {
                      chwData && chwData?.GetHealthCareWorkerById?.user.map((i: any, index: number) => {
                        return <div
                          key={i.id}
                          className="bg-primary m-1 rounded-full py-1 my-2 px-3 text-xs text-white  flex justify-center flex-row"
                        >
                  ...
                  <p>{userData?.firstName || chwData?.GetHealthCareWorkerById.fullName}</p>
                </div>
          ...
          {/* End main area */}
          {userData?.userById?.isActive || chwData?.GetHealthCareWorkerById.isActive && <Alert
            className="mt-5 mb-3"
            ...
            <form key={"formKey"} className="space-y-8 divide-y divide-gray-200">
              {
                editActive ?
                  <>
                    <div className="space-y-0">
                      {props.location.state?.component === 'chw' && <>
                        <p className='text-md py-2 mt-4'>Which kind of identification do you have for {userData?.userById?.firstName ?? chwData?.GetHealthCareWorkerById?.user?.firstName}?</p>
                            ...
                            <>
                              <div className="my-4 sm:col-span-3 w-6/12">
                                <FormField
                                  ...
                                  defaultValue={userData?.userById?.idNumber || chwData?.GetHealthCareWorkerById?.user?.idNumber}
                              ...
                              <div className="my-4 sm:col-span-3 w-6/12">
                                <FormField
                                  ...
                                  defaultValue={userData?.userById?.phoneNumber || chwData?.GetHealthCareWorkerById?.user?.phoneNumber}
                                />
                              </div>
                            </>
                        }
                  ...
                  (props.location.state?.component === 'administrators' && props.location.state?.component !== 'team-leads') ? <div className='flex flex-row justify-start pt-4 text-current'>
                    ...
                    <p className='text-xl px-4'>Email: {chwData?.GetHealthCareWorkerById?.user?.email}</p>
                    ...
                    <p className='text-xl px-4'>WhatsApp: {chwData?.GetHealthCareWorkerById}</p>
                  </div>
              }
            </form>
            {/* End main area */}
          ...
          </div>} */}
        {
          data &&
          data.tenantContext &&
          data.tenantContext.applicationName === 'GrowGreat' && props.location.state?.component !== 'administrators' && props.location.state?.component !== 'team-leads'
          &&
                ...
                dialog({
                  ...
                  render: (onSubmit: any, onCancel: any) => (
                    <AlertModal
                      ...
                      message={`${userData?.userById?.firstName || chwData?.GetHealthCareWorkerById?.user?.firstName} will lose their access to AppName immediately. Make sure you have communicated with them before deactivating them.`}
                      onCancel={onCancel}
                      onSubmit={() => {
                        onSubmit();
                        deleteUser({
