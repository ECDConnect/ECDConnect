...
export function ViewUser(props: any) {
  ...
  useEffect(() => {
    ...
    props.location.state?.component === 'chw' && props.location.state?.component === 'team-leads' && getHealthCareWorkerVisitStatus({ variables: { userId: props.location.state.userId ?? userId } });
  ...
  // SET EDIT FORMS
  useEffect(() => {
    if (userData?.userById && chwData?.GetHealthCareWorkerById && userDetailFormState) {
            ...
            <div className="p-6 dark:bg-gray-900 dark:text-gray-100 sm:p-12">
              ...
              >
                ...
                <div className='sm: pt-12'>
                  <p className='text-3xl font-normal text-black '>{userData?.userById?.fullName ?? chwData?.GetHealthCareWorkerById?.fullName}</p>
                  <div className="flex flex-row pt-4">
                    ...
                    {
                      chwData && chwData?.GetHealthCareWorkerById.roles.map((i: any, index: number) => {
                        return <div
                          key={i.id}
                          className="bg-primary m-1 rounded-full py-1 my-2 px-3 text-xs text-white  flex justify-center flex-row"
                        >
            ...
            <form key={"formKey"} className="space-y-8 divide-y divide-gray-200">
              {
                editActive ?
                  <>
                    <div className="space-y-0">
                      {props.location.state?.component === 'chw' && <>
                        <p className='text-md py-2 mt-4'>Which kind of identification do you have for {userData?.userById?.firstName ?? chwData?.GetHealthCareWorkerById?.firstName}?</p>
                            ...
                            <>
                              <div className="my-4 sm:col-span-3 w-6/12">
                                <FormField
                                  ...
                                  defaultValue={userData?.userById?.idNumber || chwData?.GetHealthCareWorkerById?.idNumber}
                              ...
                              <div className="my-4 sm:col-span-3 w-6/12">
                                <FormField
                                  ...
                                  defaultValue={userData?.userById?.phoneNumber || chwData?.GetHealthCareWorkerById?.phoneNumber}
                                />
                              </div>
                            </>
                        }
                  ...
                  (props.location.state?.component === 'administrators' && props.location.state?.component !== 'team-leads') ? <div className='flex flex-row justify-start pt-4 text-current'>
                    ...
                    <p className='text-xl px-4'>Email: {chwData?.GetHealthCareWorkerById?.email}</p>
                    <p className='text-xl px-4'>ID: {chwData?.GetHealthCareWorkerById?.idNumber}</p>
                    <p className='text-xl px-4'> Cellphone: {chwData?.GetHealthCareWorkerById?.phoneNumber}</p>
                ...
                dialog({
                  ...
                  render: (onSubmit: any, onCancel: any) => (
                    <AlertModal
                      ...
                      message={`${userData?.userById?.firstName || chwData?.GetHealthCareWorkerById?.firstName} will lose their access to AppName immediately. Make sure you have communicated with them before deactivating them.`}
                      onCancel={onCancel}
                      onSubmit={() => {
                        onSubmit();
                        deleteUser({
