...
export function ViewUser(props: any) {
  ...
  const pickQuery =(compName: string)=>{
    switch (compName) {
      case 'chw':
        return GetHealthCareWorkerByUserId;
      case 'administrators':
        return GetUserById;
      default:
        return GetUserById
  ...
  const [getUserById, { data: userData, refetch }] = useLazyQuery(GetHealthCareWorkerByUserId, {
    variables: {
      userId: '',
    },
  ...
  const [getChwById, { data:chwData, refetch }] = useLazyQuery(GetUserById, {
    variables: {
      userId: '',
    },
    fetchPolicy: 'network-only',
            ...
            <div className="p-6 dark:bg-gray-900 dark:text-gray-100 sm:p-12">
              ...
              >
                ...
                <div className='sm: pt-12'>
                  <p className='text-3xl font-normal '>{userData?.userById?.fullName}</p>
                  <div className="flex flex-row pt-4">
                    {userData && userData?.userById.roles.map((i: any, index: number) => {
                      ...
                      >
                        <p className='text-16'> {i.name === ' '?  i.name: 'chw'}</p>
                      </div>
                    })}
