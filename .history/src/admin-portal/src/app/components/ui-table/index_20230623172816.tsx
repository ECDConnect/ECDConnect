...
export default function UiTable({
  ...
  const makeRows = () => {
    ...
    return ((searchRows as any[]) || []).map((row: any, index: number) => {
      let rowKey = index;
      ...
      const cells = Object.entries(rowWithCheckbox).map(([key, value]) => (
        <td key={key}>{value}</td>
      ));
  
      return (
        <tr key={rowKey} onClick={() => viewSelectedRow(row)}>
          {cells}
        </tr>
      );
    });
      ...
      rowValue = (
        <div className="ml-1 flex cursor-pointer">
          {display_value ? (
            ...
            //   color="transparent"
            // />
            <p className="text-errorMain text-normal">
              Inactive
            </p>
          )}
        </div>
        ...
        <div className="ml-0 flex flex-row flex-wrap items-center cursor-pointer">
          {display_value &&
            display_value.map((item) => (
              <div
                key={item.id}
                className={`${item[column.displayProperty] === 'Administrator' ? 'bg-tertiary' : 'bg-primary'}` + " m-1 rounded-full py-1 px-3 text-xs text-white"}
              >
      ...
      <Table
        ...
        styling={{
          ...
          table_body: {
            ...
            table_row: 'border-none py-8 bg-infoBb cursor-pointer',
