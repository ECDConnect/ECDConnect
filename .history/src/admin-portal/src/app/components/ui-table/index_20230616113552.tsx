...
export default function UiTable({
  ...
  const makeRows = () => {
    ...
    return ((searchRows as any[]) || []).map((row: any) => {
      ...
      // Wrap the entire row content with a Link component
      return (
        <Link
          key={`row-${rowKey}`}
          to={{ pathname: `/view-user/`, state: { selectedRow: row } }}
          className="hover:bg-gray-100"
        >
          {Object.entries(rowWithCheckbox).map(([key, value]) => (
            <div key={`cell-${key}`} className="truncate w-24 px-6 pt-3 pb-3 text-sm font-medium text-gray-900 border-b border-gray-100">
              {value}
            </div>
          ))}
        </Link>
