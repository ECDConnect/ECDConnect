    ...
    <div className="table-top w-full overflow-hidden rounded-lg shadow-lg">
      <Table
        ...
        styling={{
          ...
          footer:  {
            main: `${rows.length < 10 ? 'hidden' : ''} mt-8 mx-5  `,
            statistics: {
              main: `outline-none ${
                rows.length < 10 ? 'hidden' : ''
              } text-white table-stats md:w-auto md:flex-row`,
              bold_numbers: `outline-none text-white font-bold`,
            },
            page_numbers: `outline-none page-numbers border-2 border-secondary bg-secondary z-10 text-white relative inline-flex items-center px-4 py-2 text-sm font-medium w-4 rounded-2xl mx-1`,
          },
        }}
        columns={makeColumns()}
        rows={makeRows()}
