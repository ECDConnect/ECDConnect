...
export default function UiTable({
  ...
  const viewUser = (selectedRow: any[]) => {
    history.push({
      pathname: '/view-user',
      state: {
        rowData: selectedRow
    ...
    if (typeof display_value === 'boolean') {
      ...
      rowValue =
        typeof display_value === 'string' ? (
          <div className="inline-block overflow-ellipsis" onClick={() => console.log(row)} >
            <Link to={{ pathname: `/view-user/`, state: { rowData: row } }} >{display_value}</Link>
          </div>
        ) : (
          display_value
        );
