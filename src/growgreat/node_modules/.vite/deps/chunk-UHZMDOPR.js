...
// ../../node_modules/@ionic/core/dist/esm-es5/ios.transition-7812dd37.js
var DURATION = 540;
...
var animateBackButton = function(e, t, a, r, o, n) {
  ...
  A.addElement(h);
  b.addElement(T);
  b.beforeStyles({ "transform-origin": f + " center" }).beforeAddWrite(function() {
  ...
  A.beforeStyles({ "transform-origin": d + " center" }).keyframes(g);
  e.addAnimation([b, A]);
};
...
var iosTransitionAnimation = function(e, t) {
  try {
    ...
    if (!v && y.length === 0 && p.length === 0) {
      g.addElement(f.querySelector(":scope > .ion-page, :scope > ion-nav, :scope > ion-tabs"));
    } else {
      g.addElement(v);
      g.addElement(p);
    }
    u.addAnimation(g);
    if (m) {
      g.beforeClearStyles([r]).fromTo("transform", "translateX(" + c + ")", "translateX(" + n + ")").fromTo(r, i, 1);
    } else {
      g.beforeClearStyles([r]).fromTo("transform", "translateX(" + s + ")", "translateX(" + n + ")");
    }
    if (v) {
    ...
    y.forEach(function(e2) {
      ...
      var b2 = e2.querySelector("ion-back-button");
      if (b2) {
        g2.addElement(b2);
      }
      t2.addAnimation([a2, o2, p2, y2, g2]);
      o2.fromTo(r, 0.01, 1);
      p2.fromTo(r, 0.01, 1);
      if (m) {
        if (!d2) {
        ...
        if (b2 && !C) {
          ...
          S2.addElement(shadow(b2).querySelector(".button-text")).fromTo("transform", l ? "translateX(-100px)" : "translateX(100px)", "translateX(0px)");
          t2.addAnimation(S2);
        }
      }
    });
    if (d) {
      ...
      L.forEach(function(e2) {
        ...
        var g2 = createAnimation();
        g2.addElement(shadow(e2).querySelector(".toolbar-background"));
        var b2 = createAnimation();
        var A2 = e2.querySelector("ion-back-button");
        if (A2) {
          b2.addElement(A2);
        }
        t2.addAnimation([a2, i2, p2, b2, g2]);
        u.addAnimation(t2);
        b2.fromTo(r, 0.99, 0);
        i2.fromTo(r, 0.99, 0);
        p2.fromTo(r, 0.99, 0);
        if (m) {
          ...
          g2.beforeClearStyles([r, "transform"]);
          var S2 = f2 === null || f2 === void 0 ? void 0 : f2.translucent;
          if (!S2) {
            g2.fromTo(r, "var(--opacity)", 0);
          } else {
            g2.fromTo("transform", "translateX(0px)", l ? "translateX(-100%)" : "translateX(100%)");
          }
          if (A2 && !k) {
            var T2 = createAnimation();
...
//# sourceMappingURL=chunk-UHZMDOPR.js.map
