...
import { newData } from './visitGrowthDataDay';
...
export const WeightAndLengthResultStep = ({
  ...
  const girls = newData.VisitGrowthDataDay.filter(
    (item) => item.Section === 'weight-for-age-girls'
  );
  ...
  const newObject = {
    ...
    median: {
      label: 'median',
      weight: getDataPerYear(median),
    },
    SD2: {
      label: '2 SD',
      weight: getDataPerYear(SD2),
    },
    SD3: {
      label: '3 SD',
      weight: getDataPerYear(SD3),
    },
    SD3neg: {
      label: '-3 SD',
      weight: getDataPerYear(SD3neg),
    },
    SD2neg: {
      label: '-2 SD',
      weight: getDataPerYear(SD2neg),
    },
  };
  ...
  // downloadTextFile(JSON.stringify(newObject), 'weight-for-age-girls.json')
