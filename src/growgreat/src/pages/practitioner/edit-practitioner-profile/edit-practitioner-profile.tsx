...
} from '@/pages/practitioner/edit-practitioner-profile/edit-practitioner-profile.types';
import { useOnlineStatus } from '@/hooks/useOnlineStatus';
import OnlineOnlyModal from '@/modals/offline-sync/online-only-modal';
import { authSelectors } from '@/store/auth';
import { PractitionerSetup } from '@/pages/practitioner/edit-practitioner-profile/components/practitioner-setup/practitioner-setup';
...
export function EditPractitionerProfile() {
  ...
  async function onAllStepsComplete() {
    if (isOnline) {
      if (
        userAuth?.auth_token &&
        user?.id &&
  ...
  function steps(step: EditPractitionerSteps) {
    switch (step) {
  ...
  function onBack() {
    switch (activeStep) {
      ...
      case EditPractitionerSteps.ADD_PHOTO:
        return setActiveStep(EditPractitionerSteps.ADD_PHOTO);
    }
  }
  ...
  return (
    <IonContent scrollY={true}>
      <BannerWrapper
        size={activeStep === EditPractitionerSteps.WELCOME ? 'large' : 'medium'}
        renderBorder={true}
