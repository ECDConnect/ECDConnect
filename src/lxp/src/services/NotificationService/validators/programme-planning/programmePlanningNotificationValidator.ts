...
import { differenceInMonths, getMonth, getYear, isAfter, isBefore } from 'date-fns';
...
export class ProgrammePlanningNotificationValidator implements NotificationValidator {
  ...
  getNotifications = (): Message[] => {
    const { programmeData: programmeState, user: userState } = this.store.getState();
    ...
    const programmesEndingBeforeCurrentDate = (programmeState.programmes || [])
      ...
      .sort((a, b) => (isAfter(new Date(a.endDate), new Date(b.endDate)) ? 1 : -1));

    if (programmesEndingBeforeCurrentDate.length > 0) {
      const lastPlannedDate = new Date(programmesEndingBeforeCurrentDate[0].endDate);
    ...
    if (programmesStartingAfterCurrentDate.length > 0) {
      const nextPlannedDate = programmesStartingAfterCurrentDate.reduce((prev, curr) => {
    ...
    return [
      {
        reference: `no-programmes-planned-${getMonth(this.currentDate)}-${getYear(
          this.currentDate
        )}`,
        title: `No programmes planned since ${lastPlannedDateText || 'a while'}`,
        message: 'Plan your daily routines every week to get 25 SmartStart points.',
