...
export class TraineeNotifications {
  ...
  getNotifications = () => {
    const { trainee: traineeState } = this.store.getState();

    const traineeTimeline = traineeState?.traineeOnboardTimeline;

    const overdueDate = addDays(traineeTimeline?.dayOneStartUpTrainingDate, 14);
