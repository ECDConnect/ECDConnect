...
namespace ECDLink.DataAccessLayer.Stores
{
        ...
        IUserPhoneNumberStore<ApplicationUser>
    {
        ...
        {
            var user = _context.Users.Where(x => x.Id == Guid.Parse(userId)).SingleOrDefault();
            return Task.FromResult(user);
        }

        public Task<ApplicationUser> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken)
        {
            var user = _context.Users.Where(x => x.NormalizedUserName == normalizedUserName || x.UserName == normalizedUserName).SingleOrDefault();
            return Task.FromResult(user);
        }
