...
namespace EcdLink.Api.CoreApi.GraphApi.Queries
{
    ...
    public class NotificationQueryExtension
    {
            ...
            string userId, bool inApp = true, string protocol = "")
        {
            ...
            logs.AddRange(dbRepo.GetAll().Where(x => x.To == userId && x.IsActive == true && x.ReadDate == null).ToList());//&& (x.MessageEndDate >= DateTime.Now.Date || x.MessageEndDate == null) --FE needs to make teh decision to not show, because user might have been offline for a long time and the emssages are still relevant
