...
import { DeleteRace, GetAllRace } from '@ecdlink/graphql';
...
import RacePanel from './components/race-panel/race-panel';
...
export default function RaceView() {
  ...
  useEffect(() => {
    if (data && data.GetAllRace) {
      const copyItems = data.GetAllRace.map((item) => ({
        ...item,
        _view: undefined,
        _edit: undefined,
        _url: undefined,
  ...
  const displayPanel = (item?: RaceDto) => {
    panel({
      noPadding: true,
  ...
  const deleteAndRefresh = async (item: RaceDto) => {
    dialog({
