...
import { DeleteGrant, GetAllGrant } from '@ecdlink/graphql';
...
import GrantPanel from './components/grant-panel/grant-panel';
...
export default function GrantView() {
  ...
  useEffect(() => {
    if (data && data.GetAllGrant) {
      const copyItems = data.GetAllGrant.map((item) => ({
        ...item,
        _view: undefined,
        _edit: undefined,
        _url: undefined,
  ...
  const displayPanel = (item?: GrantDto) => {
    panel({
      noPadding: true,
  ...
  const deleteAndRefresh = async (item: GrantDto) => {
    dialog({
