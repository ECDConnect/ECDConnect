...
import { DeleteEducation, GetAllEducation } from '@ecdlink/graphql';
...
import EducationLevelPanel from './components/education-level-panel/education-level-panel';

export default function EducationLevelView() {
  ...
  useEffect(() => {
    if (data && data.GetAllEducation) {
      const copyItems = data.GetAllEducation.map((item) => ({
        ...item,
        _view: undefined,
        _edit: undefined,
        _url: undefined,
  ...
  const displayPanel = (item?: EducationLevelDto) => {
    panel({
      noPadding: true,
  ...
  const deleteAndRefresh = async (item: EducationLevelDto) => {
    dialog({
