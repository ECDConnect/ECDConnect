...
export default function RelationPanel(props: RelationProps) {
  const { setNotification } = useNotifications();
  const [isEdit, setEdit] = useState(props.item ? true : false);
  const [create] = useMutation(CreateRelation);
  const [update] = useMutation(UpdateRelation);

  const { register, handleSubmit, getValues, setValue, formState } = useForm({
    resolver: yupResolver(relationSchema),
    defaultValues: initialRelationValues,
    mode: 'onChange',
  });
    ...
    if (isValid) {
      ...
      if (!isEdit) {
        await create({
          ...
          .then((response) => {
            if (response.data && response.data) {
              setNotification({
                title: 'Successfully Created Relation!',
        ...
        await update({
          ...
          .then((response) => {
            setNotification({
              title: 'Successfully Updated Relation!',
              variant: NOTIFICATION.SUCCESS,
            });
            emitCloseDialog(true);
          })
