...
import { DeleteRelation, GetAllRelation } from '@ecdlink/graphql';
...
import RelationPanel from './components/relation-panel/relation-panel';
...
export default function RelationsView() {
  ...
  const [deleteMutation] = useMutation(DeleteRelation);
  const panel = usePanel();

  useEffect(() => {
    if (data && data.GetAllRelation) {
      const copyItems = data.GetAllRelation.map((item) => ({
        ...item,
        _view: undefined,
        _edit: undefined,
        _url: undefined,
  ...
  const displayPanel = (item?: RelationDto) => {
    panel({
      noPadding: true,
  ...
  const deleteAndRefresh = async (item: RelationDto) => {
    dialog({
