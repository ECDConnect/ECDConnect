...
import { DeleteGender, GenderList } from '@ecdlink/graphql';
...
import GenderPanel from './components/gender-panel/gender-panel';
...
export default function GenderView() {
  ...
  const [deleteMutation] = useMutation(DeleteGender);
  const panel = usePanel();

  useEffect(() => {
    if (data && data.GetAllGender) {
      const copyItems = data.GetAllGender.map((item) => ({
        ...item,
        _view: undefined,
        _edit: undefined,
        _url: undefined,
  ...
  const displayPanel = (item?: GenderDto) => {
    panel({
      noPadding: true,
  ...
  const deleteAndRefresh = async (item: GenderDto) => {
    dialog({
