...
import { DeleteProvince, GetAllProvince } from '@ecdlink/graphql';
...
import ProvincePanel from './components/province-panel/province-panel';
...
export default function ProvinceView() {
  ...
  useEffect(() => {
    if (data && data.GetAllProvince) {
      const copyItems = data.GetAllProvince.map((item) => ({
        ...item,
        _view: undefined,
        _edit: undefined,
        _url: undefined,
  ...
  const displayPanel = (item?: ProvinceDto) => {
    panel({
      noPadding: true,
  ...
  const deleteAndRefresh = async (item: ProvinceDto) => {
    dialog({
