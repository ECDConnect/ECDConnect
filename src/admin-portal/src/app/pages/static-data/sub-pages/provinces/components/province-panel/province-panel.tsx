...
export default function ProvincePanel(props: ProvinceProps) {
  const { setNotification } = useNotifications();
  const [isEdit, setEdit] = useState(props.item ? true : false);
  const [create] = useMutation(CreateProvince);
  const [update] = useMutation(UpdateProvince);

  const { register, handleSubmit, getValues, setValue, formState } = useForm({
    resolver: yupResolver(provinceSchema),
    defaultValues: initialProvinceValues,
    mode: 'onChange',
  });
    ...
    if (isValid) {
      ...
      if (!isEdit) {
        await create({
          ...
          .then((response) => {
            if (response.data && response.data) {
              setNotification({
                title: 'Successfully Created Province!',
        ...
        await update({
          ...
          .then((response) => {
            setNotification({
              title: 'Successfully Updated Province!',
              variant: NOTIFICATION.SUCCESS,
            });
            emitCloseDialog(true);
          })
