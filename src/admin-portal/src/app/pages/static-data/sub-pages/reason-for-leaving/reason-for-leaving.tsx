...
import ReasonForLeavingPanel from './components/reason-for-leaving-panel/reason-for-leaving-panel';
...
export default function ReasonForLeavingView() {
  ...
  const [deleteMutation] = useMutation(DeleteReasonForLeaving);
  const panel = usePanel();

  useEffect(() => {
    if (data && data.GetAllReasonForLeaving) {
      const copyItems = data.GetAllReasonForLeaving.map((item) => ({
        ...item,
        _view: undefined,
        _edit: undefined,
        _url: undefined,
  ...
  const displayPanel = (item?: ReasonForLeavingDto) => {
    panel({
      noPadding: true,
      title: item ? `Edit ${type}` : `Create ${type}`,
      render: (onSubmit: any) => (
        <ReasonForLeavingPanel
          item={item}
  ...
  const deleteAndRefresh = async (item: ReasonForLeavingDto) => {
    dialog({
