...
export interface ReasonForLeavingPanelProps {
  item?: ReasonForLeavingDto;
...
export default function ReasonForLeavingPanel(
  ...
  const [update] = useMutation(UpdateReasonForLeaving);
    ...
    if (isValid) {
      ...
      if (!isEdit) {
        await create({
          ...
          .then((response) => {
            if (response.data && response.data) {
              setNotification({
                title: 'Successfully Created Reason for leaving!',
        ...
        await update({
          ...
          .then((response) => {
            setNotification({
              title: 'Successfully Updated Reason for leaving!',
              variant: NOTIFICATION.SUCCESS,
            });
            emitCloseDialog(true);
          })
