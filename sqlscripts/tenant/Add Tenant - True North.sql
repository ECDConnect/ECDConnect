...
do $$
declare
	tenantId 				uuid := 'dc6b770d-6898-4d8e-bbdf-f8ceeda69ece'::uuid;
	tenantOrgName			text := 'True North';
	tenantAppName			text := 'True North Connect';
	tenantSiteAddress		text := 'tnconnect.staging.ecdconnect.co.za';
	tenantAdminSiteAddress	text := 'portal-tnconnect.staging.ecdconnect.co.za';
	tenantOrgEmail			text := 'office@true-north.co.za';
	moodleCohort			text := 'true-north';
	userSuperAdmin1UserName	text := 'TNSuperAdmin1';
	userSuperAdmin1Email	text := 'peter@true-north.co.za';
	userSuperAdmin2UserName text := 'TNSuperAdmin2';
	userSuperAdmin2Email	text := 'sivu@true-north.co.za';
	...
	-- sms
	smsProvider							text := 'SMSPortal';	-- BulkSms | iTouch | SMSPortal
	smsProviderBulkSMSTokenID			text := '';
	smsProviderBulkSMSTokenSecret		text := '';
	smsProviderBulkSMSTokenBasicAuth	text := '';
	smsProviderITouchUsername			text := '';	
	smsProviderITouchPassword			text := '';
	smsProviderSMSPortalAPIKey			text := '053135be-7397-4f92-981f-23b46952c732';
	smsProviderSMSPortalAPISecret		text := '39393a1b-40a0-4ac7-baf2-4039f24f502e';
	
-- these shouldn't change
	backendAddress			text := 'api.staging.ecdconnect.co.za';
	...
	tenantBlobAddress		text := 'https://storage.staging.ecdconnect.co.za';
	...
	begin
		...
		if not exists (select * from "SystemSetting" ss where "TenantId" = tenantId) then 
			...
			(
				...
				union select tenantId, 'General.Azure', 							'General.Azure.BlobStorageConnection', 							'BlobStorageConnection', 		'DefaultEndpointsProtocol=https;AccountName=ecdconnectstoragestg;AccountKey=pqchQmYpcoDiKL6h5K+I79/6+UjEE8gNSNPWrv+P7Xj/jdjpcr3Yr3kNAA4PQ1Kcu16P6U4PI831+AStZzvtEA==;EndpointSuffix=core.windows.net'
