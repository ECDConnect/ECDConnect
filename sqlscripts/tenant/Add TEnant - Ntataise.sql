...
do $$
declare
	tenantId 				uuid := 'f5e41fb7-e586-4ed4-8ed7-2bdc72cd0207'::uuid;
	tenantOrgName			text := 'Ntataise';
	tenantAppName			text := 'NtataiseConnect';
	tenantSiteAddress		text := 'ntataise.ecdconnect.co.za';
	tenantAdminSiteAddress	text := 'portal-ntataise.ecdconnect.co.za';
	tenantOrgEmail			text := 'digitalsupport@ntataise.org';
	moodleCohort			text := 'ntataise';
	userSuperAdmin1UserName	text := 'NtataiseSuperAdmin1';
	userSuperAdmin1Email	text := 'digitalsupport@ntataise.org';
	userSuperAdmin2UserName text := 'NtataiseSuperAdmin2';
	userSuperAdmin2Email	text := 'ecd@ntataise.org';
	userSuperAdmin3UserName text := 'NtataiseSuperAdmin3';
	userSuperAdmin3Email	text := 'rika@ntataise.org';
	...
	smsProviderBulkSMSTokenID			text := 'A21BDA4A4E0147B3A966B683AD2EB487-01-A';
	smsProviderBulkSMSTokenSecret		text := 'iZWhlfcMUmA5H1SDuJcfU_s*KqgUz';
	smsProviderBulkSMSTokenBasicAuth	text := 'QTIxQkRBNEE0RTAxNDdCM0E5NjZCNjgzQUQyRUI0ODctMDEtQTppWldobGZjTVVtQTVIMVNEdUpjZlVfcypLcWdVeg==';
...
-- creating super admins
		if not exists (select * from "AspNetUsers" where "UserName" like '%SuperAdmin%' and "TenantId" = tenantId) then 
			...
			userSuperAdmin3Id := uuid_generate_v4();
			...
			"EmergencyContactFullName", "ReasonForLeaving", "ReasonForLeavingComments", "InsertedDate", "UpdatedDate", "ResetData", "RegisterType")
			values
			(userSuperAdmin3Id, NULL, NULL, userSuperAdmin3UserName, UPPER(userSuperAdmin3UserName), userSuperAdmin3Email, UPPER(userSuperAdmin3Email), true, NULL, false, false, 
			NULL, true, 0, 'AQAAAAIAAYagAAAAEKfqrFHTSXdvvw1+hULaH19dEc6HQq5GJnynylbZt/74S5BoD+NRgGOKtCjay22YFg==', 'COT6OQQLVH3EU6O24XYZ7UTQRVWL3NUK', 'abbf661f-2381-4984-a891-4b2e64c6835c', 
			true, NULL, true, '2024-01-01 22:00:00', 'Super', 'Admin3', 'Super Admin3', 'email', NULL, true, 
			'1900-01-01 00:00:00', NULL, NULL, NULL, tenantId, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 
			current_timestamp, current_timestamp, false, 'UserName');	
		else
			...
			userSuperAdmin3Id := (select "Id" from "AspNetUsers" where "TenantId" = tenantId and "UserName" = userSuperAdmin3UserName);
		end if;
...
-- assigning roles
		...
		from (select userSuperAdmin1Id "UserId" union select userSuperAdmin2Id union select userSuperAdmin3Id) u
